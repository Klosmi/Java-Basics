...
- eg.:  
  ...
  *We compare if `input` is equal to `"quit"`. We can not use the `input != "quit", because `input` is ` `String` a reference type, and we can not use comparison operators (`!=`) between reference types (technically it compares memory addresses of our String objects and not their value).*    
